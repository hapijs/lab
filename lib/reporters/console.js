...
internals.Reporter.prototype.end = function (notebook) {
    ...
    if (!failures.length) {
        output += green((notebook.tests.length - skipped.length) + ' tests complete');
        if (skipped.length) {
            output += yellow(' (' + skipped.length + ' tests skipped)');
        }
        output += '\n\n';
    }
    else {
        output += red(failures.length + ' of ' + (notebook.tests.length - skipped.length));
        if (skipped.length) {
            output += yellow(' (' + skipped.length + ' tests skipped)');
        }
        output += red(' tests failed:') + '\n\n';
