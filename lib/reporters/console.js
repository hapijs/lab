...
internals.Reporter.prototype.test = function (test) {
    ...
    if (this.settings.level === 1) {
        ...
        this.status(test.err ? this.colors.red('x') : (test.skipped || test.todo ? this.colors.magenta('-') : '.'));
    }
    else {
        ...
        if (test.err) {
            this.report(spacer + this.colors.red('\u2716' + test.id + ') ' + test.relativeTitle) + '\n');
        }
        else {
...
internals.Reporter.prototype.end = function (notebook) {
    ...
    if (this.settings.level) {
        this[this.settings.level === 1 ? 'status' : 'report']('\n\n');
    }
