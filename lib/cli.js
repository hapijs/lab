...
internals.options = function () {
    ...
    var args = {
        ...
        reporter: {
            short: 'r',
            type: 'string',
            description: 'reporter type [console, html, json, tap, lcov]',
            default: 'console'
        },
        silence: {
            short: 's',
    ...
    var parser = Yargs.usage('Usage: lab [options] [path]');
    var opts = Object.keys(args);
    for (var i = 0, il = opts.length; i < il; ++i) {
        var key = opts[i];
        var opt = args[key];

        parser = parser.alias(key, opt.short)
            .describe(key, opt.description);

        if (opt.default !== undefined) {
            parser = parser.default(key, opt.default);
        }
    ...
    options.coverage = (options.coverage || options.threshold > 0 || options.reporter === 'html' || options.reporter === 'lcov');
    if (options.globals) {
