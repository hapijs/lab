...
exports.run = async function () {
    ...
    if (settings.node) {
        const nodeTestRunner = require('node:test');
        const { tap } = require('node:test/reporters');

        const { promise, resolve, reject } = Promise.withResolvers();

        const testFiles = await internals.findTestFiles(settings.paths, settings);

        console.info(settings, testFiles)
...
internals.findTestFiles = async function (paths, options) {
...
internals.traverse = async function (paths, options) {
    ...
    const testFiles = await internals.findTestFiles(paths, options);
...
internals.options = function () {
    ...
    const definition = {
        ...
        node: {
            type: 'boolean',
            description: 'use node test runner',
            default: false
        },
        output: {
            alias: 'o',
    ...
    const keys = ['assert', 'bail', 'colors', 'context-timeout', 'coverage', 'coverage-exclude',
        ...
        'lint-errors-threshold', 'lint-fix', 'lint-options', 'lint-warnings-threshold', 'linter', 'node',
