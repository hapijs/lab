...
internals.options = function () {
    ...
    var keys = ['coverage', 'colors', 'dry', 'debug', 'environment', 'flat', 'grep', 'globals', 'timeout', 'parallel',
        'reporter', 'threshold', 'context-timeout', 'sourcemaps', 'lint', 'linter', 'transform', 'lint-options',
        'lint-errors-threshold', 'lint-warnings-threshold'];
    ...
    if (Array.isArray(options.reporter)) {
        if (argv.output && options.output.length !== options.reporter.length) {
            console.error(Bossy.usage(definition, 'lab [options] [path]'));
            process.exit(1);
        }
    }
