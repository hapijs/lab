...
exports.console = function (emitter, options) {
    ...
    emitter.on('test', function (test) {
        ...
        if (test.err) {
            failures.push(test);
            process.stdout.write(internals.color('#red[x]'));
        }
        else {
            process.stdout.write('.');
        }
    ...
    internals.consoleSummary(emitter, options);
};


exports.spec = function (emitter, options) {
    ...
    emitter.on('startExperiment', function(experiment) {
        var format = internals.color(currentIndent  + '#black[%s]');
        console.log(format, experiment.relativeTitle);
        currentIndent += indent;
    });

    emitter.on('endExperiment', function(experiment) {
        currentIndent = currentIndent.slice(indent.length);
    });

    emitter.on('test', function (test) {
        var format = internals.color(currentIndent + (test.err ? '#red[x]' : '#green[✓]')  + ' #gray[%s]');
        console.log(format, test.relativeTitle);
    });
...
internals.consoleSummary = function (emitter, options) {
    ...
    emitter.once('end', function (result) {
        ...
        var formatSummary = function () {
            ...
            for (var i = 0, il = failures.length; i < il; ++i) {
                ...
                if (typeof test.err.actual === 'string' &&
                    ...
                    msg = msg.replace(/^/gm, '      ');
                    format = internals.color('  #black[%s) %s:\n%s]\n' + (test.timeout ? '' : '#gray[%s]\n'));
                }
                else {
        ...
        var formatGlobals = function () {
            ...
            console.log(internals.color('#gray[The following leaks were detected]\n'));
            for (var i = 0, il = result.leaks.length; i < il; ++i) {
                console.log(internals.color('#red[' + result.leaks[i] + ']\n'));
            }
        };
        ...
        var colorError = function (str) {
            ...
            if (str.added || str.removed) {
                return str.value.split('\n').map(function (segment) {
                    ...
                    return internals.color((str.added ? '#green-bg' : '#red-bg') + '[' + segment + ']');
                }).join('\n');
            }
