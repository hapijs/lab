...
internals.coverage = function (filename, data) {
    ...
    data.source.forEach(function (line, num) {
        ...
        if (data.lines[num] === 0) {
        ...
        else if (data.lines[num] !== undefined) {
            ret.sloc++;
            ...
            if (data.statements[num]) {
                ...
                chunks.push({ source: line.slice(from), miss: mask[a] });

                if (isMiss) {
                    ret.source[num].branch = chunks;
                    ret.misses++;
                }
                else {
                    ret.hits++;
...
internals.formatBranch = function (index, src, statements, offset, lineNum, branches) {

    var statement = statements[index];
    var loc = statement.loc;

    branches.push({
        source: src.slice(index, loc.start.column - offset)
    });

    if (statements.length > index + 1 &&
        statements[index + 1].loc.start.line === lineNum &&
        statements[index + 1].loc.start.column - offset < statements[index].loc.end.column - offset) {

        internals.formatBranch(index + 1, src.slice(loc.start.column - offset, loc.end.column - offset), statements, loc.start.column - offset, lineNum, branches);
        statements[index + 1] = statements[index + 2];
        statements.length--;
    }
    else {
        branches.push({
            miss: !statements[index].hit,
            source: src.slice(loc.start.column - offset, loc.end.column - offset)
        });
    }
};
