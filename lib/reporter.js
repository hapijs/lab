...
exports.console = function (emitter, options) {
    ...
    emitter.on('end', function (ms, errors) {
        ...
        if (errors && errors.length > 0) {
            console.log(color('\n\n #green[The following unhandled errors occurred while executing the tests]\n'));
            errors.forEach(function (err) {
                var format = color('#gray[%s]\n');
                console.error(format, err.stack || err.message);
            });
        }
    });
};
