...
exports.Console = exports.console = function (engine) {
        ...
        failures.forEach(function (test, i) {
            ...
            if (typeof test.err.actual === 'string' &&
                ...
                msg = Diff[type](test.err.actual, test.err.expected).map(function (str) {
                    ...
                    if (str.added) {
                        return internals.colorLines('diff added', str.value);
                    }
                    
                    if (str.removed) {
                        return internals.colorLines('diff removed', str.value);
                    }
                    
                    return str.value;
                }).join('');

                msg = '\n' + internals.color('diff removed', 'actual') + ' ' + internals.color('diff added', 'expected') + '\n\n' + msg + '\n';
                msg = msg.replace(/^/gm, '      ');
                format = internals.color('error title', '  %s) %s:\n%s\n') + (test.timeout ? '' : internals.color('error stack', '%s\n'));
            }
...
internals.colorLines = function(name, str) {
