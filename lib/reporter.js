...
exports.spec = function (emitter, options) {
    ...
    emitter.on('test', function (test) {
        ...
        if (test.err) {
            ...
            process.stdout.write(color('#green[>] #black[' + test.title + ']\n\n'));
        }
    });
    ...
    emitter.once('end', function (result) {
        ...
        var checkErrors = function () {
            ...
            console.error('');
            console.error(color('\n\n #red[' + failures + ' of ' + count + ' tests failed]#gray[:]\n'));
    ...
    var formatError = function (test) {
        ...
        console.error(format, test.title, msg, (test.timeout ? '' : stack));
    };
