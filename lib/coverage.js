...
exports.instrument = function (options) {
    ...
    if (options.transform) {
        options.transform.extensions.forEach(function (element) {
            
            internals.extensions.push(element);
        });

        internals.transform = options.transform.transform;
    }

    internals.extensions.forEach(function (element) {

        internals.prime(element);
    });
};
...
internals.instrument = function (filename) {
    ...
    if (internals.transform) {
        content = internals.transform(content);
    }
