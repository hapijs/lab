...
exports.instrument = function (options) {
    ...
    require.extensions['.js'] = function (localModule, filename) {
        ...
        localModule._compile(internals.instrument(filename.replace(/\\/g, '/'), options), filename);
    };
};
...
internals.instrument = function (filename, options) {
    ...
    var preamble =
        ...
        'var __$$labLine = function (filename, line) {' +
            globalVar + '[filename].lines[line]++;' +
        '};' +

        'var __$$labStatement = function (filename, id, line, source) {' +

            'var statement = ' + globalVar + '[filename].statements[line][id];' +
            'if (!statement.bool) {' +
    ...
    tracking.forEach(function (item) {
        ...
        preamble += globalVar + '[\'' + filename + '\'].lines[' + item + '] = 0;';
    });

    statements.forEach(function (item) {

        preamble +=
            globalVar + '[\'' + filename + '\'].statements[' + item.line + '] = ' + globalVar + '[\'' + filename + '\'].statements[' + item.line + '] || {};' +
            globalVar + '[\'' + filename + '\'].statements[' + item.line + '][' + item.id + '] = { hit: {}, bool: ' + item.bool + ', loc: ' + item.loc + '};';
    });
...
exports.analyze = function (notebook, options) {
    ...
    var report = global[options.global || internals.global] || {};
    var cov = {
        sloc: 0,
        hits: 0,
        misses: 0,
