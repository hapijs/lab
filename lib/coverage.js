...
internals.global = '__$$labCov';
internals.origLoader = require.extensions['.js'];


exports.instrument = function (options) {

    var loader = function (localModule, filename) {

        var pattern = require.extensions['.js'].pattern;
        if (pattern &&
            pattern.regex.test(filename)) {

            return localModule._compile(internals.instrument(filename, pattern.options), filename);
        }
    ...
    loader.pattern = {
        regex: new RegExp('^' + internals.escape(process.cwd()) + '\\' + Path.sep + '(?!test|node_modules)(.+)$'),
        options: options || {}
    };
