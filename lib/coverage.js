...
internals.prime = function () {
    ...
    require.extensions['.js'] = function (localModule, filename) {
        ...
        for (var i = 0, il = internals.patterns.length; i < il; ++i) {
            if (internals.patterns[i].regex.test(filename)) {
                return localModule._compile(internals.instrument(filename, internals.patterns[i].options), filename);
            }
        }
...
exports.instrument = function (options) {
    ...
    var base = internals.escape(options.path || process.cwd());
    var exclude = [].concat(options.exclude || ['test', 'node_modules']).map(internals.escape).join('|');

    internals.patterns.unshift({
        regex: new RegExp('^' + base + '\\' + Path.sep + (exclude ? '(?!' + exclude + ')' : '') + '.+$'),
        options: options || {}
    });
};
...
internals.instrument = function (filename, options) {

    filename = filename.replace(/\\/g, '/');

    var globalVar = options.global || internals.global;
    ...
    var annotate = function (node, parent) {
        ...
        if (trackedTypes.indexOf(node.type) !== -1 &&
            ...
            node.set(globalVar + '._line(\'' + filename + '\',' + node.loc.start.line + ');' + node.source());
        }
            ...
            node.set(node.test.source() + '?' + globalVar + '._statement(\'' + filename + '\',' + consequent + ',' + line + ',' + node.consequent.source() + ') : ' + globalVar + '._statement(\'' + filename + '\',' + alternate + ',' + line + ',' + node.alternate.source() + ')');
        }
            ...
            node.set(globalVar + '._statement(\'' + filename + '\',' + left + ',' + line + ',' + node.left.source() + ')' + node.operator + globalVar + '._statement(\'' + filename + '\',' + right + ',' + line + ',' + node.right.source() + ')');
        }
        else if (node.parent &&
            ...
            node.set(globalVar + '._statement(\'' + filename + '\',' + test + ',' + line + ',' + node.source() + ')');
        }
    };
        ...
        'if (typeof ' + globalVar + ' === \'undefined\') {' +
            globalVar + ' = {' +
                ...
                '_line: function (filename, line) {' +
                    ...
                    globalVar + '.files[filename].lines[line]++;' +
                '},' +

                '_statement: function (filename, id, line, source) {' +

                    'var statement = ' + globalVar + '.files[filename].statements[line][id];' +
                    'if (!statement.bool) {' +
    ...
    tracking.forEach(function (item) {
        ...
        preamble += globalVar + '.files[\'' + filename + '\'].lines[' + item + '] = 0;';
    });

    statements.forEach(function (item) {

        preamble +=
            globalVar + '.files[\'' + filename + '\'].statements[' + item.line + '] = ' + globalVar + '.files[\'' + filename + '\'].statements[' + item.line + '] || {};' +
            globalVar + '.files[\'' + filename + '\'].statements[' + item.line + '][' + item.id + '] = { hit: {}, bool: ' + item.bool + ', loc: ' + item.loc + '};';
    });
...
exports.analyze = function (notebook, options) {
    ...
    var globalVar = options.global || internals.global;
    var report = global[globalVar] ? global[globalVar].files : {};
    var cov = {
        sloc: 0,
        hits: 0,
        misses: 0,
    ...
    cov.tests = notebook.tests.map(clean);
