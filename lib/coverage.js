...
global[internals._state] = global[internals._state] || { modules: new Set(), externals: new Set() };        // $lab:coverage:ignore$
            ...
            tree.comments.map((node) => {
                ...
                for (let i = start; i <= end; ++i) {
                    ...
                    if ((!nodesByLine[i] || !nodesByLine[i].find((type) => type !== 'Line' && type !== 'Block')) && !record.commentedLines[i]) {
                        ...
                        if (i === start || i === end || !blank.test(internals.state.sources[filename][i - 1])) {
                            commented++;
                        }
                    }
                }
...
exports.analyze = async function (options) {
    ...
    const report = global.__$$labCov || { files: {} };          // $lab:coverage:ignore$
    ...
    for (let i = 0; i < files.length; ++i) {
        ...
        if (pattern.test(filename)) {
            ...
            report.files[filename].source = internals.state.sources[filename] || [];
