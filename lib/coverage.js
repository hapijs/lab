...
internals.instrument = function (filename) {
    ...
    var annotate = function (node, parent) {
        ...
        if (node.type === 'LogicalExpression') {
            ...
            node.set('__$$labStatement(\'' + filename + '\',' + right + ',' + line + ',' + node.left.source() + ')' + node.operator + '__$$labStatement(\'' + filename + '\',' + right + ',' + node.right.source() + ')');
        }
    };
    ...
    var __$$labBranch = function (filename, line, column, source) {

        __$$labCov[filename].branches[line][column].hit = !!source;
        return source;
    };
    ...
    branching.forEach(function (item) {
        ...
        preamble +=
            '__$$labCov[\'' + filename + '\'].branches[' + item.line + '] = __$$labCov[\'' + filename + '\'].branches[' + item.line + '] || [];' +
            '__$$labCov[\'' + filename + '\'].branches[' + item.line + '][' + item.column + '] = { hit: false, consequent: ' + item.consequent + ', alternate: ' + item.alternate + ' };';
    });
