...
internals.addSourceMapsInformation = function (ret, num) {
    ...
    let originalPosition = SourceMapSupport.mapSourcePosition(position);
    ...
    if (source.chunks) {
        source.chunks.forEach((chunk) => {
            ...
            originalPosition = SourceMapSupport.mapSourcePosition({
                source: ret.filename,
                line: num,
                column: chunk.column
            });
            ...
            // We need to force originalColum to 0 for first element, or we loose indentation
            chunk.originalColumn = chunk.column === 0 ? 0 : originalPosition.column;
        });
    }
};
