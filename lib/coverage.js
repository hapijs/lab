...
internals.prime = function () {
    ...
    require.extensions['.js'] = function (localModule, filename) {
        ...
        for (var i = 0, il = internals.patterns.length; i < il; ++i) {
            if (internals.patterns[i].regex.test(filename.replace(/\\/g, '/'))) {
...
exports.instrument = function (options) {
    ...
    var settings = {
        path: options.path || internals.defaults.path,
        exclude: options.exclude || (options.path ? [] : internals.defaults.exclude),
        coverageVar: options.coverageVar || internals.defaults.coverageVar
    };

    internals.patterns.unshift({
        regex: internals.pattern(settings),
        options: settings
    });
};
...
exports.load = function (filename, options) {
    ...
    var temp = Path.join(Os.tmpDir(), [Date.now(), process.pid, Crypto.randomBytes(8).toString('hex')].join('-'));
    Fs.writeFileSync(temp, Fs.readFileSync(filename));
    internals.temps.push(temp);
    exports.instrument({ coverageVar: options.coverageVar, path: temp });
    return require(temp);
};
...
exports.cleanup = function () {
    ...
    internals.temps.forEach(function (filename) {
...
internals.pattern = function (options) {
    ...
    var base = internals.escape(options.path);
    var exclude = [].concat(options.exclude).map(internals.escape).join('|');
    var regex = '^' + base + (exclude ? (base[base.length - 1] === '/' ? '' : '\\/') + '(?!' + exclude + ')' : '');
...
internals.instrument = function (filename, options) {
    ...
    var coverageVar = options.coverageVar;
...
exports.analyze = function (options) {
    ...
    var coverageVar = options.coverageVar || internals.defaults.coverageVar;
    var report = global[coverageVar] ? global[coverageVar].files : {};
    var cov = {
        sloc: 0,
        hits: 0,
        misses: 0,
