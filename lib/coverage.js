...
internals.instrument = function (filename) {
    ...
    const annotate = function (node, parent) {
        ...
        else if (node.type === 'Line' || node.type === 'Block' && node.value) {
            node.set(`global.__$$labCov._comment('${filename}', ${line}, ${node.loc.end.line}); ${node.source()};`);
        }
    };
    ...
    if (typeof global.__$$labCov === 'undefined') {
        global.__$$labCov = {
            ...
            _comment: function (name, startLine, endLine) {

                const report = global.__$$labCov.files[name];
                // Count commented lines to avoid adding hits on them
                for (let i = startLine; i <= endLine; ++i) {
                    // But only if the comment doesn't share line with a statement or a full line
                    if (!(i in report.statements) && !(i in report.lines)) {
                        report.commentedLines[i] = 1;
                    }
                }
            }
        };
    ...
    if (typeof global.__$$labCov.files[filename] === 'undefined') {
        ...
        record.sloc = internals.sources[filename].filter((line) => !/^\s*$/.test(line)).length -
            tree.comments.map((node) => {

                const start = node.loc.start.line;
                return nodesByLine[start].find((type) => type !== 'Line' && type !== 'Block')
                     ? 0
                     : node.loc.end.line - start + 1;
            }).reduce((a, b) => a + b);
    }
