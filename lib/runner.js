...
exports.execute = function (experiments, options) {
    ...
    internals.parallel = options.parallel;
    ...
    var count = internals.count(experiments, idsFilter, grep);          // Sets test.id
...
internals.count = function (experiments, ids, grep, _seq) {
    ...
    if (experiments) {
        for (var e = 0, el = experiments.length; e < el; ++e) {
            ...
            counter += internals.count(experiment.experiments, ids, grep, seq);
        }
    }
