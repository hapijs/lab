...
internals.reporting = false;

exports.report = function (experiments, options, callback) {

    callback = callback || process.exit;

    if (internals.reporting) {
        return;
    }

    internals.reporting = true;
    ...
    var dest = settings.output ? Fs.createWriteStream(settings.output) : process.stdout;
