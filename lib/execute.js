...
exports.execute = function (skipTraverse) {
    ...
    var argv = Optimist.usage('Usage: lab [options] [path]')
        ...
        .alias('r', 'reporter')
        .default('r', 'console')
        .describe('r', 'reporter module [console, coverage, html, json, spec, summary, tap, threshold]')
        ...
        .alias('v', 'verbose')
        .default('v', false)
        .describe('v', 'verbose output for coverage and console reporters')
        .boolean('v')
    ...
    // start     - (plan)           execution start
    // test      - (test)           test results
    // end       - (notebook)       execution complete
    ...
    Reporters[argv.r](emitter, options);
    ...
    emitter.emit('start', { tests: Lab.index });
    ...
    function (err) {
        ...
        emitter.emit('end', { ms: Date.now() - startTime, leaks: leaks, tests: report.tests });
        process.exit((report.failures || leaks) ? 1 : 0);
    });
};
