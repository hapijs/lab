...
exports.execute = function () {
    ...
    var argv = Optimist.usage('Usage: lab [-r reporter] [-o filename] [-t threshold] [-m timeout] [-e NODE_ENV]')
        .default('r', 'console')
        .default('t', 100)
        .default('m', 2000)
        .default('e', 'test')
    ...
    if (['html', 'coverage', 'threshold'].indexOf(argv.r) !== -1) {
        ...
        blanket.options('filter', new RegExp(filterPattern, 'gi'));
        blanket.options('onlyCwd', true);
        blanket.options('data-cover-flags', { branchTracking: true });
    }
