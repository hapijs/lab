...
Chai.config.includeStack = true;                                     // Show the stack where the issue occurs
...
internals.experiment = function (title /*, options, fn */) {
    ...
    if (settings.only) {
        Assert(!this._current.only.experiment, 'Cannot set multiple experiments at the same level as only');
...
internals.test = function (title /*, options, fn */) {
    ...
    if (settings.only) {
        Assert(!this._current.only.test, 'Cannot set multiple tests at the same level as only');
