...
try {
    internals.config = require('./config');
} catch (e) {
    internals.config = {};
}

internals.config = Hoek.applyToDefaults(internals.default, internals.config);


// Schedule automatic execution when used without the CLI unless directed not to

if (internals.config.executeOnLoad) {
